...
import { Instagram, Facebook, Music2, Check, Clock } from "lucide-react";
...
export function SocialFollowCard({
  ...
  function statusFor(platform: string) {
    return requestedPlatforms.find((r) => r.platform === platform)?.status;
  }
  ...
  return (
    <div className="rounded-2xl border border-line bg-white p-6 shadow-card">
      ...
      <p className="mt-1.5 text-xs text-slate-500">
        Suis un compte MatchCV, clique sur le bouton correspondant : ton crédit est ajouté après vérification
      ...
      <div className="mt-4 space-y-2">
        {PLATFORMS.map((p) => {
          ...
          const isPending = pending.has(p.id) && status !== "rejected";
          return (
